...
class SubAdminUsersController extends Controller
{
    public function index($id)
    {
        ...
        if($id==1)
        {
           return view('subadmin.users.user.index')->with('users', User::orderBy('updated_at', 'DESC')->get())->with('id',1);
        }
        if($id==2)
        {
            // dd($id);
           return view('subadmin.users.user.index')->with('users', User::orderBy('updated_at', 'DESC')->get())->with('id',2);
        }
        if($id==3)
        {
           return view('subadmin.users.user.index')->with('users', User::orderBy('updated_at', 'DESC')->get())->with('id',3);
        }
        if($id==4)
        {
           return view('subadmin.users.user.index')->with('users', User::orderBy('updated_at', 'DESC')->get())->with('id',4);
        }
    }
...
return redirect()->route('subadmin.users',['id' => 1])->with('users', User::all())->with('id',1)->with('success','Client Created Successfully');
       }
       if($role==2){
    ...
    return redirect()->route('subadmin.users',['id' => 3])->with('users', User::all())->with('id',3)->with('success','University Created Successfully');
     ...
     return redirect()->route('subadmin.users',['id' => 2])->with('users', User::all())->with('id',2)->with('success','Consultant Created Successfully');
       }
       if($role==5){
        return User::create([
            'first_name' => $request->first_name,
     ...
     return redirect()->route('subadmin.users',['id' => 4])->with('users', User::all())->with('id',4)->with('success','SubAdmin Created Successfully');
       }
