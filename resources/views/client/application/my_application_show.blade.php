            ...
            <div class="table-responsive" >
                <table class="table table-hover table-striped">
                    ...
                    <tr>
                        ...
                        <td>{{$application->booking->userConsultant->first_name ?? ''}} {{$application->booking->userConsultant->last_name ?? ''}}</td>
                    </tr>
                    <tr>
                        <th scope="row">Consultant Address</th>
                        <td>{{$application->booking->userConsultant->address ?? ''}}</td>
                    </tr>

                    <tr>
                        <th scope="row">Consultant Mobile No.</th>
                        <td>{{$application->booking->userConsultant->mobile ?? ''}}</td>
                    </tr>

                    <tr>
                        <th scope="row">Consultant E-mail</th>
                        <td>{{$application->booking->userConsultant->email ?? ''}}</td>
                    </tr>

                    <tr>
                        <th scope="row">Consultant Country</th>
                        @if(isset($application->booking->userConsultant->countries_id))
                        <?php $country = DB::table('countries')->where('countries_id',$application->booking->userConsultant->countries_id)->get()->first();?>
                        @endif
            ...
            <div class="table-responsive" >
                <table class="table table-hover table-striped" >
                    ...
                    <tr>
                        ...
                        @endif
                        <td> {{$country->countries_name ?? ''}} </td>
                        {{-- <td>{{$application->booking->userConsultant->country->countries_name ?? ''}}</td> --}}
                    </tr>
                </div>
...
<div class="col-lg-12">
    <div class="row clearfix">
        <div class="col-md-12">
            ...
            <div class="body">
                <form action="{{ route('client.application.document.store')}}" method="POST" enctype="multipart/form-data" >
                    @csrf
        ...
        <div class="card-header" id="headingTwo">
          <h2 class="mb-0">
            <button class="btn btn-link btn-block text-left collapsed" type="button" data-toggle="collapse" data-target="#collapse-{{$key}}" aria-expanded="false" aria-controls="collapse-{{$key}}">
                <span style="color: #17C2D7;font-weight: 400;">{{$applied->userUniversity->university->university_name ?? ''}} ({{$applied->course->title ?? ''}})  <span style="float: right;"><i class="fa fa-arrow-down"> </i></span></span>
            </button>
          </h2>
        </div>
 ...
 $(document).on('click', '#applyAccepted', function ()
 {
       ...
       if(appliedUniversityRowIdAccepted > 0){
         $.ajaxSetup({headers:
             ...
             $.ajax({
                     type: "post",
                     url: "{{route('client.application.offer.accepted')}}",
 ...
 <script>
    $('.closes').click( function()
    {
        ...
        $.ajaxSetup({
                ...
                $.ajax({
                            type: "post",
                            url: "{{route('client.application.document.destroy')}}",
