               ...
               @endif
                    <div class="chatapp_body" style="margin-right: 0px;">
                            ...
                            <div class="row clearfix">
                                <div class="col-lg-12">
                                    <div class="chat-about">
                                        <h6 class="m-b-0" id="hed">@if($check == NULL) Select User to Start Conversation @elseif($check->send_by==4) {{ $useme->first_name }} {{ $useme->last_name }} (Last Message) Click on the user to start conversation @else {{ $usemeSend->first_name }} {{ $usemeSend->last_name }} (Last Message) Click on the user to start conversation @endif</h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="chat-history ">
                            ...
                            @if(isset($check) && $check->send_by==4 && $check != NULL)
...
console.log(userid)
                ...
                $.ajax({
                  url: "{{ route('client.messenger.fetchdata') }}",
                    method: "POST",
...
$(".msg_history").stop().animate({
     scrollTop: $(".msg_history")[0].scrollHeight
       ...
       $.ajax({
              type: "POST",
              url: "{{ route('client.messenger.sendmessage') }}",
